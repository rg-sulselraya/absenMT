...
async function renderTeacherHome() {
  ...
  $('#page-content').innerHTML = `${pageHeading(`Halo, ${escapeHtml(state.user.name)}!`, 'Pantau status kehadiran dan lakukan absensi Anda hari ini.', `<button class="primary-button" data-action="go-scan"><span class="scan-glyph">⌗</span> Scan QR Absensi</button>`)}
    ...
    <div class="teacher-home">
      ...
      <section class="card status-card"><h3>Status absensi</h3><div class="status-row"><span>Jam masuk</span><strong class="${masuk ? 'done-in' : ''}">${masuk ? `✓ ${masuk.time}` : 'Belum scan'}</strong></div><div class="status-row"><span>Lokasi masuk</span><span>${masuk ? statusPill(masuk) : '—'}</span></div><div class="status-row"><span>Jam pulang</span><strong class="${pulang ? 'done-out' : ''}">${pulang ? `✓ ${pulang.time}` : 'Belum scan'}</strong></div><div class="status-row"><span>Lokasi pulang</span><span>${pulang ? statusPill(pulang) : '—'}</span></div></section>
    </div>
