...
function route(method, rawPath, parameter, input) {
  try {
    ...
    if (!session) return apiError('Belum login.', 'UNAUTHORIZED', 401);
    if (path === '/api/config' && method === 'GET') return configResponse();
    ...
    if (path === '/api/my-attendance' && method === 'GET') return requireTeacher(session, function() { return myAttendance(session.userId, params.date || today()); });
    if (path === '/api/attendance/scan' && method === 'POST') return requireTeacher(session, function() { return submitAttendance_(session, input || {}); });
...
function configResponse() { var branches = getBranches(); var activeId = getProperty('ACTIVE_BRANCH_ID') || (branches[0] && branches[0].id) || ''; return success({ settings: { activeBranchId: activeId, timezone: getTimezone(), appName: 'Master Teacher Attendance' }, branch: findById(branches, activeId) || branches[0] || null, sheetsEnabled: true, sheetsProvider: 'google-apps-script', spreadsheetIdConfigured: Boolean(getProperty('SPREADSHEET_ID')), serverTime: nowIso() }); }
function getMasterTeachers() { return readRows('Master Teacher').map(function(r) { var raw = valueString(r.value(['status'])); return { id: valueString(r.value(['id mt', 'id master teacher', 'id'])), name: valueString(r.value(['nama master teacher', 'nama', 'name'])), branchId: valueString(r.value(['cabang utama', 'id cabang', 'branch id', 'branchid'])), status: activeValue(raw) ? 'active' : 'inactive', statusLabel: raw || '—', createdAt: valueString(r.value(['created at', 'dibuat', 'created'])) }; }).filter(function(t) { return t.id; }); }
function getBranches() { return readRows('Branches').map(function(r) { var lat = numberValue(r.value(['latitude'])); var lon = numberValue(r.value(['longitude'])); var id = valueString(r.value(['id cabang', 'branch id', 'id'])); var status = valueString(r.value(['status'])); var radius = numberValue(r.value(['radius (meter)', 'radius', 'radius meter'])); return { id: id, name: valueString(r.value(['nama cabang', 'nama', 'name'])), address: valueString(r.value(['alamat', 'address'])), latitude: validCoordinate(lat, lon) ? lat : null, longitude: validCoordinate(lat, lon) ? lon : null, latitudeRaw: valueString(r.value(['latitude'])), longitudeRaw: valueString(r.value(['longitude'])), radius: radius === null ? 50 : radius, nearRadius: 200, outsideRadius: 500, active: activeValue(status), status: status, qrPayload: valueString(r.value(['qr payload', 'payload'])) || id }; }).filter(function(b) { return b.id; }); }
function getAttendance() { return readRows('Attendance').map(function(r) { var status = valueString(r.value(['status lokasi', 'status'])); return { attendanceId: valueString(r.value(['id absensi', 'attendance id', 'id'])), date: normalizeDate(r.value(['tanggal', 'date'])), mtId: valueString(r.value(['id mt', 'id master teacher'])), mtName: valueString(r.value(['nama master teacher', 'nama'])), branchId: valueString(r.value(['id cabang', 'branch id'])), branchName: valueString(r.value(['nama cabang', 'cabang'])), type: valueString(r.value(['jenis absensi', 'jenis', 'type'])), timestamp: valueString(r.value(['timestamp'])), time: valueString(r.value(['jam', 'time'])), latitude: numberValue(r.value(['latitude'])), longitude: numberValue(r.value(['longitude'])), distanceMeters: numberValue(r.value(['jarak (meter)', 'jarak', 'distance'])), locationKey: locationKey(status), locationStatus: status, deviceId: valueString(r.value(['device id', 'deviceid'])), createdAt: valueString(r.value(['created at'])) }; }).filter(function(r) { return r.attendanceId; }); }
...
function submitAttendance_(session, input) { if (!allowAttempt_('scan:' + session.userId + ':' + session.deviceId, 8, 60)) return apiError('Terlalu banyak percobaan scan. Tunggu sebentar lalu coba lagi.', 'RATE_LIMITED', 429); var device = findDevice(session.userId, session.deviceId); if (!device || device.status !== 'approved') return apiError('Perangkat ini belum diotorisasi Admin.', 'DEVICE_PENDING', 403, { deviceId: session.deviceId }); var branch = findById(getBranches(), String(input.branchId || input.payload || '').trim()); if (!branch) return apiError('QR tidak valid atau cabang tidak ditemukan.', 'INVALID_QR', 400); if (!branch.active) return apiError('Cabang ini sedang tidak aktif.', 'BRANCH_INACTIVE', 400); if (!validCoordinate(branch.latitude, branch.longitude)) return apiError('Koordinat cabang belum dikonfigurasi Admin.', 'BRANCH_COORDINATES_MISSING', 409); var lat = numberValue(input.latitude), lon = numberValue(input.longitude); if (!validCoordinate(lat, lon)) return apiError('GPS tidak tersedia atau koordinat tidak valid. Aktifkan lokasi lalu coba lagi.', 'GPS_INVALID', 400); var distance = Math.round(haversine(lat, lon, branch.latitude, branch.longitude) * 10) / 10; var status = locationStatus(distance, branch), stamp = dateParts(new Date()); var day = getAttendance().filter(function(r) { return sameId(r.mtId, session.userId) && r.date === stamp.date; }).sort(function(a, b) { return new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime(); }); if (day.length >= 2) { var log = { logId: Utilities.getUuid(), date: stamp.date, time: stamp.time, timestamp: nowIso(), mtId: session.userId, mtName: session.name, branchId: branch.id, branchName: branch.name, result: 'complete', message: 'Absensi hari ini sudah lengkap.', deviceId: session.deviceId, latitude: lat, longitude: lon, distanceMeters: distance, locationKey: status.key, locationStatus: status.label, createdAt: nowIso() }; appendScanLog_(log); return success({ complete: true, message: log.message, log: log }); } var record = { attendanceId: 'ATT-' + new Date().getTime() + '-' + Utilities.getUuid().slice(0, 6).toUpperCase(), date: stamp.date, mtId: session.userId, mtName: session.name, branchId: branch.id, branchName: branch.name, type: day.length ? 'PULANG' : 'MASUK', timestamp: nowIso(), time: stamp.time, latitude: lat, longitude: lon, distanceMeters: distance, locationKey: status.key, locationStatus: status.label, deviceId: session.deviceId, createdAt: nowIso() }; appendAttendanceRecord_(record); return success({ complete: false, record: record, warning: status.key === 'far' ? 'Absensi tetap tercatat. Lokasi scan berada jauh dari titik cabang.' : null }); }
function saveTeacherPin_(id, pin) { if (!/^\d{6}$/.test(String(pin || ''))) return apiError('PIN harus terdiri dari tepat 6 digit.', 'INVALID_PIN_FORMAT', 400); var teacher = findById(getMasterTeachers(), id); if (!teacher) return apiError('ID Master Teacher tidak ditemukan di Google Sheet.', 'TEACHER_NOT_FOUND', 404); if (teacher.status !== 'active') return apiError('Akun Master Teacher tidak aktif.', 'TEACHER_INACTIVE', 409); setProperty_('MTA_PIN_' + idKey(teacher.id), hashSecret(pin)); return success({ teacher: teacher, pinConfigured: true }); }
