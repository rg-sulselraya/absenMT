...
function getAttendance() { return readRows('Attendance').map(function(r) { var status = valueString(r.value(['status lokasi', 'status'])), timestamp = valueString(r.value(['timestamp'])); return { attendanceId: valueString(r.value(['id absensi', 'attendance id', 'id'])), date: normalizeDate(r.value(['tanggal', 'date']), timestamp), mtId: valueString(r.value(['id mt', 'id master teacher'])), mtName: valueString(r.value(['nama master teacher', 'nama'])), branchId: valueString(r.value(['id cabang', 'branch id'])), branchName: valueString(r.value(['nama cabang', 'cabang'])), type: valueString(r.value(['jenis absensi', 'jenis', 'type'])), timestamp: timestamp, time: normalizeTime(r.value(['jam', 'time'])), latitude: numberValue(r.value(['latitude'])), longitude: numberValue(r.value(['longitude'])), distanceMeters: numberValue(r.value(['jarak (meter)', 'jarak', 'distance'])), locationKey: locationKey(status), locationStatus: status, deviceId: valueString(r.value(['device id', 'deviceid'])), createdAt: valueString(r.value(['created at'])) }; }).filter(function(r) { return r.attendanceId; }); }
...
function getAttendanceForDay_(mtId, date) { var sheet = getSpreadsheet().getSheetByName('Attendance'); if (!sheet) throw new Error('Sheet "Attendance" tidak ditemukan.'); var lastRow = sheet.getLastRow(), lastColumn = Math.max(sheet.getLastColumn(), ATTENDANCE_COLUMN_COUNT); if (lastRow < 2) return []; var headers = sheet.getRange(1, 1, 1, lastColumn).getValues()[0].map(function(v) { return String(v == null ? '' : v).trim().toLowerCase(); }); var dateColumn = findHeaderIndex_(headers, ['tanggal', 'date']), timestampColumn = findHeaderIndex_(headers, ['timestamp']), mtColumn = findHeaderIndex_(headers, ['id mt', 'id master teacher']); if (dateColumn < 0 || mtColumn < 0) return []; var rowCount = lastRow - 1, dates = sheet.getRange(2, dateColumn + 1, rowCount, 1).getValues(), timestamps = timestampColumn < 0 ? [] : sheet.getRange(2, timestampColumn + 1, rowCount, 1).getValues(), mtIds = sheet.getRange(2, mtColumn + 1, rowCount, 1).getValues(), matches = []; for (var i = 0; i < rowCount; i++) if (normalizeDate(dates[i][0], timestampColumn < 0 ? '' : timestamps[i][0]) === date && sameId(mtIds[i][0], mtId)) matches.push(i + 2); if (!matches.length) return []; return matches.map(function(rowNumber) { return attendanceFromRow_(headers, sheet.getRange(rowNumber, 1, 1, lastColumn).getValues()[0]); }).filter(function(r) { return r.attendanceId; }).sort(function(a, b) { return new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime(); }); }
function attendanceFromRow_(headers, row) { var value = function(names) { var i = findHeaderIndex_(headers, names); return i < 0 ? '' : row[i]; }, status = value(['status lokasi', 'status']), timestamp = valueString(value(['timestamp'])); return { attendanceId: valueString(value(['id absensi', 'attendance id', 'id'])), date: normalizeDate(value(['tanggal', 'date']), timestamp), mtId: valueString(value(['id mt', 'id master teacher'])), mtName: valueString(value(['nama master teacher', 'nama'])), branchId: valueString(value(['id cabang', 'branch id'])), branchName: valueString(value(['nama cabang', 'cabang'])), type: valueString(value(['jenis absensi', 'jenis', 'type'])), timestamp: timestamp, time: normalizeTime(value(['jam', 'time'])), latitude: numberValue(value(['latitude'])), longitude: numberValue(value(['longitude'])), distanceMeters: numberValue(value(['jarak (meter)', 'jarak', 'distance'])), locationKey: locationKey(status), locationStatus: valueString(status), deviceId: valueString(value(['device id', 'deviceid'])), createdAt: valueString(value(['created at'])) }; }
...
function normalizeDate(v, fallback) { if (v instanceof Date) { var formatted = Utilities.formatDate(v, getTimezone(), 'yyyy-MM-dd'); if (formatted !== '1899-12-30' && formatted !== '1900-01-01') return formatted; } else { var s = valueString(v), match = s.match(/(\d{4}-\d{2}-\d{2})/); if (/^\d+(?:\.\d+)?$/.test(s) && Number(s) > 1) return Utilities.formatDate(new Date(Date.UTC(1899, 11, 30) + Number(s) * 86400000), getTimezone(), 'yyyy-MM-dd'); if (match && match[1] !== '1899-12-30' && match[1] !== '1900-01-01') return match[1]; } var fallbackText = fallback instanceof Date ? Utilities.formatDate(fallback, getTimezone(), 'yyyy-MM-dd') : String(fallback == null ? '' : fallback), fallbackMatch = fallbackText.match(/(\d{4}-\d{2}-\d{2})/); return fallbackMatch ? fallbackMatch[1] : ''; }
