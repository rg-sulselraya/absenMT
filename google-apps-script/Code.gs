...
function attendanceFromRow_(headers, row) { var value = function(names) { var i = findHeaderIndex_(headers, names); return i < 0 ? '' : row[i]; }, status = value(['status lokasi', 'status']); return { attendanceId: valueString(value(['id absensi', 'attendance id', 'id'])), date: normalizeDate(value(['tanggal', 'date'])), mtId: valueString(value(['id mt', 'id master teacher'])), mtName: valueString(value(['nama master teacher', 'nama'])), branchId: valueString(value(['id cabang', 'branch id'])), branchName: valueString(value(['nama cabang', 'cabang'])), type: valueString(value(['jenis absensi', 'jenis', 'type'])), timestamp: valueString(value(['timestamp'])), time: valueString(value(['jam', 'time'])), latitude: numberValue(value(['latitude'])), longitude: numberValue(value(['longitude'])), distanceMeters: numberValue(value(['jarak (meter)', 'jarak', 'distance'])), locationKey: locationKey(status), locationStatus: valueString(status), deviceId: valueString(value(['device id', 'deviceid'])), createdAt: valueString(value(['created at'])) }; }
